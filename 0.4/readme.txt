...
Only the start up scripts and whatever else I point out is mine.
...
the first thing you want to do is make sure you have all the necessary files and scripts, (Currenly 3 scripts)
Start, init and res.
...
RUN the "Start" script (double click), the first prompt you get will ask you if you want to change
memory allocation, so type "Y" and hit enter, it will then open up a notepad file and you will see
this line:
...
If you have just finished allocating the memory, the window should still be open, so just enter "N" this time,
if not, just open the script and do what I just said.
...
For the starter plugins inside the pack, there is an option in the start script that will allow you
to launch the config for them, E.g Essentials, PEX or WorldGuard. (NYI)
